...
void thread_schedule(void) {
    ...
    int min_run_time = __INT_MAX__; // 设置为最大整数，寻找最小运行时间的线程
    ...
    for (int i = 0; i < MAX_THREAD; i++) {
        ...
        if (t->state == RUNNABLE && t->enter_time <= current_time) {
            // 如果该线程的运行时间小于当前最小运行时间，则选择该线程
            if (t->run_time < min_run_time) {
                min_run_time = t->run_time;
                next_thread = t;
                printf("Selected thread with id %d and run time %d\n", next_thread->id, next_thread->run_time);
            }
        }
    }
